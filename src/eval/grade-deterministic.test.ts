...
test("wroteFile passes on a matching path and fails when the path is missing", () => {
  const a: Assertion = { kind: "wroteFile", path: "out.txt", regex: false };
  assert.equal(gradeOne(a, { writes: [{ path: "out.txt", content: "x" }] }).pass, true);
  const miss = gradeOne(a, { writes: [{ path: "other.txt", content: "x" }] });
  assert.equal(miss.pass, false);
...
test("wroteFile with contentMatches passes on matching content and fails on mismatch", () => {
  ...
  assert.equal(gradeOne(a, { writes: [{ path: "out.txt", content: "hello world" }] }).pass, true);
  const mismatch = gradeOne(a, { writes: [{ path: "out.txt", content: "goodbye" }] });
